...
Change Log
DATE		VER		WHO			WHAT
07/07/13	0.1		PC		Modified and merge all MCP2515
libraries found. Stripped away most unused functions and corrected MCP2515 defs
09/12/13	0.2		PC		Added selectable CS SPI for CAN
controller to use 1 IC to control several mcp2515 02/05/14	0.3
PC		Added filter and mask controls 05/01/14	0.4		PC
Cleaned up functions, variables and added message structures for J1939, CANopen
and CAN. 05/07/14	1.0		PC		Released Library to the
public through GitHub 06/18/14  1.9   NEM   Preparing a unified CAN library
across three different CAN controllers
-------------------------------------------------------------------------------------------------------------
Features:
CAN V2.0B
...
Designed to be used with ATMEL ATMega328P with Arduino bootloader, MCP2515
...
void CAN_MCP2515::begin(uint32_t bitrate, uint8_t mode) {
  ...
  reset();     // Set MCP2515 into Config mode by soft reset. Note MCP2515 is in
               // Config mode by default at power up.
  clearRxBuffers();
  clearTxBuffers();
  clearFilters();
...
CAN_Frame CAN_MCP2515::read() {
  ...
  if (message.extended == CAN_EXTENDED_FRAME) {
    ...
    message.id |= ((uint32_t)(RXBnSIDL & MCP2515_SIDL_SID)
                   << 13); // ID<20:18> = SIDL<7:5>
    message.id |= ((uint32_t)(RXBnSIDL & MCP2515_SIDL_EID)
                   << 16); // ID<17:16> = SIDL<1:0>
    // Serial.println(message.id, HEX);
...
// This functions provides an easy way to see the message if user doesn't care
// about the actual message protocol. No message struct is used here.
void CAN_MCP2515::read(uint32_t *ID, uint8_t *length_out, uint8_t *data_out) {
  ...
  memcpy(data_out, message_to_be_parsed.data,
         sizeof(message_to_be_parsed.data));
}
...
uint8_t CAN_MCP2515::write(const CAN_Frame &message) {
  uint8_t TXBnSIDH, TXBnSIDL, TXBnEID8, TXBnEID0, TXBnDLC, msgStatus,
      loadBuffer, sendBuffer;
  msgStatus = readStatus();

  if (!(msgStatus &
        MCP2515_STATUS_TXB0CNTRL_TXREQ)) // transmit buffer 0 is open
  {
    loadBuffer = MCP2515_LOAD_TX_BUFFER_0_ID;
    sendBuffer = MCP2515_RTS_TXB0;
  } else if (!(msgStatus &
               MCP2515_STATUS_TXB1CNTRL_TXREQ)) // transmit buffer 1 is open
  {
    loadBuffer = MCP2515_LOAD_TX_BUFFER_1_ID;
    sendBuffer = MCP2515_RTS_TXB1;
  } else if (!(msgStatus &
               MCP2515_STATUS_TXB2CNTRL_TXREQ)) // transmit buffer 2 is open
  {
  ...
  if (message.extended == CAN_EXTENDED_FRAME) {
    ...
    TXBnSIDL |=
        ((message.id >> 16) & MCP2515_SIDL_EID); // SIDL<1:0> = ID<17:16>
...
// Function to load and send any message. (J1939, CANopen, CAN). It assumes user
// knows what the ID is supposed to be
uint8_t CAN_MCP2515::write(uint32_t ID, uint8_t frameType, uint8_t length,
                           uint8_t *data) // changed from send() to write()
{
...
uint8_t CAN_MCP2515::readRXStatus() {
  ...
  Values are as follows
    ...
    | 0 | 1 |---|---|---|---|---|---| Message in RXB0
    | 1 | 0 |---|---|---|---|---|---| Message in RXB1
    | 1 | 1 |---|---|---|---|---|---| Message in both buffers* (Buffer 0 has
  higher priority, therefore, RXB0 status is reflected in bits 4:0)
...
// MCP2515 can be set into 5 different modes: CONFIG, NORMAL, SLEEP, LISTEN,
// LOOPBACK
void CAN_MCP2515::setMode(uint8_t mode) {
  modifyAddress(MCP2515_CANCTRL, MCP2515_REQOPn,
                mode); // Writes config values to registers
}
...
void CAN_MCP2515::setMask(uint8_t mask, uint8_t b0, uint8_t b1, uint8_t b2,
                          uint8_t b3) {
...
// Set Receive Filters. Will think of a more user friendly way to set these in
// the future but for right now it works....
void CAN_MCP2515::setFilter(uint8_t filter, uint8_t b0, uint8_t b1, uint8_t b2,
                            uint8_t b3) {
  setMode(MCP2515_MODE_CONFIG);
  digitalWrite(CS, LOW);
  SPI.transfer(filter);
...
// At power up, MCP2515 buffers are not truly empty. There is random data in the
// registers This loads buffers with zeros to prevent incorrect data to be sent.
...
// Note: If RTS is sent to a buffer that has all zeros it will still send a
...
// Enable hardware Request to send pins. It allows messages to be send by
// driving RTS pins low. These are not to be confused with RTS commands. These
// are the actual MCP2515 hardware pins
void CAN_MCP2515::enableRTSPins() {
  // According to section 10.1, TXRTSCTRL is only modifiable in Configuration
  // Mode
  setMode(MCP2515_MODE_CONFIG);
...
// Enable interrupts. The CANINTF register contains the corresponding interrupt
// flag bit for each interrupt source. When an interrupt occurs, the INT pin is
// driven low by the MCP2515 and will remain low until the interrupt is cleared
// by the MCU. An interrupt can not be cleared if the respective condition still
// prevails.
